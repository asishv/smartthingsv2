...
SEARCH_RESPONSE = 'HTTP/1.1 200 OK\r\nCACHE-CONTROL:max-age=30\r\nEXT:\r\nLOCATION:%s\r\nSERVER:Linux, UPnP/1.0, Pi_Contact_Sensor/1.0\r\nST:%s\r\nUSN:uuid:%s::%s'
...
class StatusServer(resource.Resource):
    """HTTP server that serves the status of the door to the
       SmartThings hub"""
    isLeaf = True
    def __init__(self, device_target, subscription_list, door_status):
        self.device_target = device_target
    ...
    def render_GET(self, request): # pylint: disable=invalid-name
        ...
        if request.path == '/status':
            if self.door_status['last_state'] == 'closed':
                cmd = 'status-closed'
            else:
                cmd = 'status-open'
...
class DoorMonitor(object):
    """Monitors a door status, generating notifications whenever its
       state changes"""
    def __init__(self, device_target, subscription_list, polling_freq, gpio_pin, door_status): # pylint: disable=too-many-arguments
        self.device_target = device_target
        self.subscription_list = subscription_list
        ...
        reactor.callLater(self.polling_freq, self.check_state, None) # pylint: disable=no-member

    def get_current_state(self):
        """Get the current state of the door as a string"""
        if self.gpio_pin < 0:
            ...
            if self.countdown <= 0:
                ...
                if self.door_status['last_state'] == 'closed':
                    ...
                    return 'closed'
            return self.door_status['last_state']
        else:
            # Read GPIO pin
            import RPi.GPIO as GPIO
    ...
    def check_state(self, _):
        """Called periodically to check if the door has changed state"""
        current_state = self.get_current_state()
        if current_state != self.door_status['last_state']:
            self.door_status['last_state'] = current_state
            self.notify_hubs()
...
def main():
    ...
    arg_proc = argparse.ArgumentParser(description='Provides a door open/closed status to a SmartThings hub')
    ...
    arg_proc.add_argument('--pollingfreq', dest='polling_freq', help='Number of seconds between polling door state', default=1, type=int)
    ...
    subscription_list = {}
    door_status = {'last_state': 'unknown'}


    # SSDP server to handle discovery
    SSDPServer(status_port=options.http_port, device_target=device_target)

    # Monitor door state and send notifications on state change
    DoorMonitor(device_target=device_target,
    ...
    status_site = server.Site(StatusServer(device_target, subscription_list, door_status))
